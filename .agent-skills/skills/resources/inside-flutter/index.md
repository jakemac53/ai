...
One of the most distinctive aspects of Flutter is its _aggressive_
_composability_. Widgets are built by composing other widgets,
...
interface, which is computed during _layout_ and used during _painting_
and
...
Flutter aims for linear performance for initial layout, and _sublinear_
_layout performance_ in the common case of subsequently updating an
...
object will not be visited again[1](#fn-1)
...
as a result, each render object is visited _at most twice_ [2](#fn-2)
during layout:
...
position in the parent's coordinate system[3](#fn-3).
...
- A render object can declare that it uses the constraints provided
   ...
   _even if the constraints are not tight_ and _even if the parent's_
  _layout depends on the child's size_, because the child cannot change
...
_tree_, which retains the logical structure of the user interface.
...
by induction, all its ancestor elements are also clean[4](#fn-4).
...
- If each list contains a widget with the same key[5](#fn-5),
...
- **Child-model agnostic.** Unlike most toolkits, which use child lists,
   ...
   method rather than a concrete `firstChild` and `nextSibling`
   interface.
...
to the header, the list, or the grid[6](#fn-6).
...
widgets on demand _as long as those widgets are descendants of the_
_render object currently performing layout_.
...
an entirely different type, `TextSpan`. At the `RenderParagraph`
boundary,
...
uncovers the `Spacer` widget, which uses `Expanded` and `SizedBox`
directly
...
constructor does not allow the `down` property of `PointerEvent`
to
...
reactive paradigm[7](#fn-7)
to manipulate the underlying rendering tree.
...
where 0.0 represents the `start` and 1.0 represents the `end` [8](#fn-8),
...
and therefore the interface instead defines `lerpFrom` and `lerpTo`
methods,
...
1. For layout, at least. It might be revisited
    ...
    and for hit testing if necessary. [↩](#fnref-1)
...
2. Reality, of course, is a bit more
    ...
    common case of shrink-wrapping. [↩](#fnref-2)
...
3. Technically, the child's position is not
    ...
    the computation entirely if they are not subsequently painted. [↩](#fnref-3)
...
6. For accessibility, and to give applications
    ...
    widgets for a few hundred pixels before and after the visible widgets. [↩](#fnref-6)

7. This approach was first made popular by
    Facebook's React library. [↩](#fnref-7)
...
Unless stated otherwise, the documentation on this site reflects Flutter 3.38.6. Page last updated on 2025-10-30. [View source](https://github.com/flutter/website/blob/main/src/content/resources/inside-flutter.md) or [report an issue](https://github.com/flutter/website/issues/new?template=1_page_issue.yml&page-url=https://docs.flutter.dev/resources/inside-flutter&page-source=https://github.com/flutter/website/blob/main/src/content/resources/inside-flutter.md "Report an issue with this page").