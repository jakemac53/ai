---
name: adding-ios-app-extensions
...
following steps show how to add a [Share](https://developer.apple.com/library/archive/documentation/General/Conceptual/ExtensibilityPG/Share.html)
...
2. In the console, open the Xcode workspace for the
...
3. In Xcode, add an app extension called `Share`
...
4. In Xcode, change the order of the build process.
   ...
   - Drag **Embed Foundation Extensions** above
      **Run Script**.
5. Make sure your **Minimum Deployments** iOS value is properly
...
6. In the console, run the following command to rebuild your
    iOS app:
...
7. [Test your app with the simulator](#test-extensions).
...
- [Simulator](#172-tab-panel)
- [Physical device](#173-tab-panel)

1. In Xcode, [add an app extension to your project](#add-extension).

2. In the console, use the following command to run your
    iOS app:
...
the [Core Spotlight](https://developer.apple.com/documentation/corespotlight)
framework indexes your app,
...
check out [Leveraging Apple's System APIs and Frameworks](/platform-integration/ios/apple-frameworks)

or search [pub.dev](https://pub.dev/packages).
...
4. Choose which App Group you want to add the target from
    one of two options:
...
- **Key/value:** Use the [`shared_preference_app_group`](https://pub.dev/packages/shared_preference_app_group)

   plugin to read or write to `UserDefaults` within the same App Group.

- **File:** Use the App Group container path from the
   [`path_provider`](https://pub.dev/packages/path_provider) plugin to
...
- **Database:** Use the App Group container path from
   ...
   plugin to create a database with the
...
use the [`workmanager`](https://pub.dev/packages/workmanager)
plugin.
...
you can use [Deep Linking](/ui/navigation/deep-linking).
...
in a [Share](https://developer.apple.com/library/archive/documentation/General/Conceptual/ExtensibilityPG/Share.html)
extension. To support a scrollable list in
...
extension, with a `FlutterViewController`.
...
a [Share](https://developer.apple.com/library/archive/documentation/General/Conceptual/ExtensibilityPG/Share.html)
app extension. In this example the app extension
...
1. [Add an extension to your Flutter app](#add-extension) if you haven't
    already done so.

2. In the console, navigate to your Flutter project
...
4. In Xcode, add the pre-action to the
...
6. (Optional) In Xcode, replace any storyboard files with
...
7. In Xcode, update the `ShareViewController` to use the
...
- [UIKit-Swift](#174-tab-panel)
- [UIKit-ObjC](#175-tab-panel)
...
2. In Xcode, add `GeneratedPluginRegistrant.m` to the
   ...
   - Click **Add**.
3. (Swift only) In Xcode, update the
...
4. In Xcode, update the code for `ShareViewController`
...
- [UIKit-Swift](#176-tab-panel)
- [UIKit-ObjC](#177-tab-panel)
...
The [home\_widget](https://pub.dev/packages/home_widget) package provides a large amount of functionality,
...
- [Render Flutter widgets](https://docs.page/abausg/home_widget/features/render-flutter-widgets)
   in an app extension as an image.

- [Save and retrieve data](https://docs.page/abausg/home_widget/usage/sync-data) from `UserDefaults`
   on iOS.
